function findAndReplace(expression, replacement, file)
%
%   Protip from Ben: to do a recursive search/replace, only enter the first
%   two arguments. The third will default to dir('**/*.m')
%
% find_and_replace(to_find, to_replace, file)
% 
...
% Modified 2020 Benjamin Hershberg
    ...
    for k = 1:length(file)
        ...
        fid = fopen(fullfile(folder{k},file{k}));
        ...
        if(~isequal(text,textOrig))
            fprintf('Match found in %s\n',fullfile(folder{k},file{k}));
            % Write out the new file.
            fid = fopen(fullfile(folder{k},file{k}), 'w');
            fwrite(fid, text);
            fclose(fid);
        end
