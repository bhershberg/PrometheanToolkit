function [results, options] = fft_analysis_standalone(data, options)
% JL: 2019-12-02 Created from fft_analysis.m by Benjamin Hershberg
    ...
    if(getFftSize > 0), fft_size = getFftSize;
    else fft_size = floor(log10(length(data))/log10(2)); end
...
%	JL : commented this to avoid user interaction
