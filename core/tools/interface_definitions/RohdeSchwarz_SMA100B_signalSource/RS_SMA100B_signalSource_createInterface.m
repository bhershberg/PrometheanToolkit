function interfacePanel = RS_SMA100B_signalSource_createInterface(interfacePanel)
    ...
    if(interfacePanel.UserData.interfaceIndex <= 0) 
        ...
        interface.type = 'RS SMA100B Signal Source';
        % -----------------------------------------------------
        ...
        interface.mode = 'Normal'; % Normal, Low Noise, Low Distortion
        interface.harmonic_filter = 0;      % 1, 0        
        % -----------------------------------------------------
    ...
    col = [1.7 2.3];
    popup_Mode = placeDropdown(interfacePanel,row+2,col,{'Normal','Low Noise','Low Distortion'},editBoxOptions);
    popup_Mode.Value = find(strcmp(popup_Mode.String,interface.mode));
    
    col = [2.3 2.8];
    checkbox_Filter = placeCheckbox(interfacePanel,row+2,col,'Harmonic Filter',interface.harmonic_filter,editBoxOptions);
    ...
    interfacePanel.UserData.menu_mode = popup_Mode;
    interfacePanel.UserData.checkbox_harmonic_filter = checkbox_Filter;
