        ...
        switch interface.connect_method
            ...
            case 'LAN'
                if ~contains(interface.IP,',')
                    ...
                    IP1 = extractBefore(interface.IP,',');
                    IP2 = extractAfter(interface.IP,',');
                    if ~isfield(interface,'CPhi')
                        interface.CPhi = 0;
                    end
                    if ~isfield(interface,'PLevD') % Difference in power level
                        interface.PLevD = 0;
                    end
                    SMB_setSig_AllInOne('CF',interface.signal_frequency, ...
                        'PLev',interface.signal_power-3+interface.PLevD/2, ...
                        'RFOn',interface.enable, ...
                        'CPhi',interface.CPhi/2,...
                        'GPIBAddr',IP1, ...
                        'HarmRej',interface.harmonic_filter, ...
                        'NormMode',mode_normal, 'LOWD',mode_low_Dist, 'LOWN', mode_low_Noise); 
                    SMB_setSig_AllInOne('CF',interface.signal_frequency, ...
