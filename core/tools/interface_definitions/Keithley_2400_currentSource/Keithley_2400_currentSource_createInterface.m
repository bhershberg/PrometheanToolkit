function interfacePanel = Keithley_2400_currentSource_createInterface(interfacePanel)
    ...
    if(interfacePanel.UserData.interfaceIndex <= 0) 
        ...
        interface.type = 'Keithley 2400 Current Source';
        % -----------------------------------------------------
        
        % *** STEP 3 ***
        % PLACE INTERFACE-SPECIFIC FIELDS HERE:----------------
        interface.current = 0;
        interface.voltage_limit = 1;
        interface.filter = 0;
    ...
    col = [1 1.5];
    placeText(interfacePanel,row,col,'Current (uA):',textBoxOptions);
    col = [1.5 1.8];
    textbox_current = placeEdit(interfacePanel,row,col,sprintf('%0.12g',1e6*interface.current),editBoxOptions);
    
    col = [1 1.5];
    placeText(interfacePanel,row+1,col,'Vdc Limit (mV):',textBoxOptions);
    col = [1.5 1.8];
    textbox_voltageLimit = placeEdit(interfacePanel,row+1,col,sprintf('%0.12g',interface.voltage_limit*1e3),editBoxOptions);
