function TXdata = send_spi_cfg_slaveSel(slaveID,slowFact,KRNOCid,varargin)
% spi_cfg_slaveSel Send a select command to one of the SPI's slave.
% - INPUTS
...
%   cfg_SA_v = 7, slave address
%   SPIcfgSetupSingleSlaveNodeAccess(cfg_SG_v*32+cfg_SA_v,master_SPIclk, SPI_CLK, SPI_CSN, SPI_MOSI, SPI_MISO);
%       - invokes:
...
%       SPIcfg("100","000",SG,SA,clk_in,sclk_out,cs_n_out,mosi_out,miso_in);
%           - invokes:
%           CMD   = 100
%           Dummy = 000
%           SG    = bit 7 to 5 (MSB   to MBS-2) of slaveID=(cfg_SG_v*32+cfg_SA_v), group address
