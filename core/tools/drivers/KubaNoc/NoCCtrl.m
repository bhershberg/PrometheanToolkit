function CTRL_STRUCT = NoCCtrl(varargin)
...
try
  ...
  % ------------------------- Reset -------------------------------------- %
    if RST == 1
      ...
      clear NocRST NocCE NocDi NocTE
  ...
  % ---------------- Individual control download ------------------------- %     
    if exist('d_load_value','var')==1 || exist('d_load_offset_value','var')==1 
      ...
      if tofile == 0
        ...
        NocTE = [1 0];
        downloadCtrl('NocDi',NocDi,'NocTE',NocTE,'portConfig',tofile,'clkd',clkd,'p00',p00,'p01',p01,'p07',p07,'devname',devname)
        clear NocDi NocTE
...
% ------------------------------------------------------------------------ %
  ...
  % dictionary.
    ...
    % NocCE: active high -> 1 if not provided - pre-pended
